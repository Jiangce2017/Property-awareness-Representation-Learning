...
class Model(nn.Module):
    def __init__(self,batchsize,num_properties, x_dim, hidden_dim, latent_dim,device,model_type,im_x,im_y,modes1, modes2):
        ...
        self.modes2 = modes2
        if model_type == 'CNN':
            self.Encoder = CNN_Encoder(num_properties, input_dim=1, hidden_dim=hidden_dim, latent_dim=latent_dim,im_x=im_x, im_y=im_y)
            self.Decoder = CNN_Decoder(latent_dim=latent_dim, hidden_dim = hidden_dim, output_dim = x_dim,im_x=im_x, im_y=im_y)
        elif model_type == 'FL':
            self.Encoder = FL_Encoder(num_properties, input_dim=x_dim, hidden_dim=hidden_dim, latent_dim=latent_dim)
            self.Decoder = FL_Decoder(latent_dim=latent_dim, hidden_dim = hidden_dim, output_dim = x_dim)
        elif model_type == 'FNO':
            self.Encoder = FNO_Encoder(num_properties,input_dim=x_dim, hidden_dim=hidden_dim, latent_dim=latent_dim,im_x=im_x, im_y=im_y,modes1=modes1,modes2=modes2)
...
class CNN_Encoder(nn.Module):
        def __init__(self,num_properties, input_dim, hidden_dim, latent_dim,im_x,im_y):
            ...
            self.conv3 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=(3, 3), stride=1, padding=1)
            self.conv4 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=(3, 3), stride=1, padding=1)
            self.conv5 = nn.Conv2d(hidden_dim, latent_dim, kernel_size=(3, 3), stride=1, padding=1)
            self.LeakyReLU = nn.LeakyReLU(0.2)
            self.im_x = im_x
            self.im_y = im_y
            self.num_properties = num_properties
        ...
        def forward(self, x):
            ...
            sph_err = _calculate_spherical_error(x,mid_value)
        ...
        def reparameterization(self, mid_value,batchsize, channels, size_x, size_y,device):
            ...
            x_grid, y_grid, dx, dy = get_spherical_gird(batchsize, channels, size_x, size_y, device)
            z_grid = torch.sqrt(2 - x_grid**2 - y_grid**2)
...
class CNN_Decoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim, output_dim,im_x,im_y):
        ...
        self.conv1 = nn.Conv2d(latent_dim, hidden_dim, kernel_size=(3, 3), stride=1, padding=1)
        self.conv2 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=(3, 3), stride=1, padding=1)
        self.conv3 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=(3, 3), stride=1, padding=1)
        self.conv4 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=(3, 3), stride=1, padding=1)
        self.conv5 = nn.Conv2d(hidden_dim, 1, kernel_size=(3, 3), stride=1, padding=1)
        self.LeakyReLU = nn.LeakyReLU(0.2)
        self.hidden_dim = hidden_dim
        self.im_x = im_x
        self.im_y = im_y
