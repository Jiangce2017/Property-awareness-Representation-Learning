...
class Model(nn.Module):
    def __init__(self,num_properties, x_dim, hidden_dim, latent_dim,device,model_type,im_x,im_y,modes1, modes2):
...
class CNN_Encoder(nn.Module):
        def __init__(self,num_properties, input_dim, hidden_dim, latent_dim,im_x,im_y):
            ...
            self.layer_variance = nn.Linear(hidden_dim, latent_dim-num_properties)
            self.LeakyReLU = nn.LeakyReLU(0.2)
            self.im_x = im_x
            self.im_y = im_y
            self.num_properties = num_properties
